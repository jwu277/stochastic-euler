...
def main():
    ...
    ax.plot(elpts[:,0], elpts[:,1], ls='-', c='blue', label='Generated FPE')
    ...
    ax.scatter(v0, w0 - psic, marker='x', c='g', label='Psi Cutoff Point')

    # 7b. Alpha point
    alpha = 0.0
    ax.scatter(*mll.new2og(np.array([cdist * np.array([np.cos(alpha), np.sin(alpha)])]), np.array([0]))[0], marker='x', c='orange', label=f'$\\alpha = {alpha}$')
